...
# Run the 7 scripts IN THIS ORDER (timeouts match the grader):
...
amls_project/
├── README.md                 # this file
├── DEVELOPMENT.md            # full dev log + retrospective (dev-only, not submitted)
...
└── solution/                 # the pipeline (this folder is submitted)
    ├── Dockerfile
    ├── requirements.txt
    ├── common.py             # shared constants + features + CNN + calibration (MANDATORY)
    ├── clean.py  prepare.py  train.py  predict.py
    ├── train_augmented.py    predict_augmented.py
    ├── explain.py            # Task 1.4
    ├── data/                 # RUNTIME ONLY — do not commit / do not submit
    └── artifacts/            # RUNTIME ONLY — regenerated by the scripts
```

`data/`, `artifacts/`, and `__pycache__/` are git-ignored (see `.gitignore`).
For the submission zip, **exclude `data/` and `artifacts/`** — the grader regenerates them.
